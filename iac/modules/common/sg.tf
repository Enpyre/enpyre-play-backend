resource "aws_security_group" "pubnic_alb_sg" {
  name        = "pubnic-production-alb-security-group"
...
resource "aws_security_group_rule" "nsg_lb_ingress_rule" {
  ...
  security_group_id = aws_security_group.pubnic_alb_sg.id

  depends_on = [aws_security_group.pubnic_alb_sg]
}
